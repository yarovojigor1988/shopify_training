...
class engravingVariantSelects extends HTMLElement {
  constructor() {
    super();
    this.engravingInput = document.getElementById('engraving-message');
    this.addEventListener('change', this.onVariantChangeA);
  }
  ...
  updateOptions() {
    ...
    if(!constraint.test(engravingMessageInput.value.trim()) || engravingMessageInput.value.trim().length === 0) {
      ...
      engravingLength = engravingMessageInput.value.replaceAll(' ', '').length.toString();
    }    
    this.options = [];
    this.options.push(engravingLength);
...
customElements.define('engraving-message', engravingVariantSelects);
